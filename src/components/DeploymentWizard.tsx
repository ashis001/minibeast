...
const DeploymentWizard = () => {
  ...
  const handleModuleSelect = async (moduleId: string) => {
    ...
    try {
      const response = await fetch(`https://goat-attach-grid-adaptive.trycloudflare.com/api/deployment/status/${moduleId}`);
  ...
  const handleRedeploy = async () => {
    setIsClearing(true);
    try {
      const response = await fetch(`https://goat-attach-grid-adaptive.trycloudflare.com/api/deployment/clear/${selectedModule}`, {
        method: 'DELETE'
      });
      const data = await response.json();
