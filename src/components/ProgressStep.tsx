...
const ProgressStep = ({ onComplete, deploymentId }: ProgressStepProps) => {
  ...
  useEffect(() => {
    ...
    const pollDeploymentStatus = async () => {
      try {
        const response = await fetch(`http://64.227.183.35/api/deployment/${deploymentId}/status`);
  ...
  const retryDeployment = async () => {
    ...
    try {
      const response = await fetch(`http://64.227.183.35/api/deployment/${deploymentId}/retry`, {
        method: 'POST'
      });
