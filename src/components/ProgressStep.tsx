...
const ProgressStep = ({ onComplete, deploymentId }: ProgressStepProps) => {
  ...
  useEffect(() => {
    ...
    const pollDeploymentStatus = async () => {
      try {
        const response = await fetch(`https://goat-attach-grid-adaptive.trycloudflare.com/api/deployment/${deploymentId}/status`);
  ...
  const retryDeployment = async () => {
    ...
    try {
      const response = await fetch(`https://goat-attach-grid-adaptive.trycloudflare.com/api/deployment/${deploymentId}/retry`, {
        method: 'POST'
      });
